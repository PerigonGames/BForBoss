...
Shader "Perigon/URPUnlitShaderTessallated"
{
...
#pragma domain domain
	...
	Varyings vert(Attributes input)
	{
		...
		float NoiseSmaller = tex2Dlod(_Noise, float4(input.uv * 2, 0, 0)).r;
...
#define DomainPos(fieldName) v.fieldName = \
	...
	half4 frag(Varyings IN) : SV_Target
	{
		...
		float NoiseSmaller = tex2Dlod(_Noise, float4(IN.uv * 2, 0, 0)).r;
		float combinedNoise = (Noise + NoiseSmaller) * 0.5;
